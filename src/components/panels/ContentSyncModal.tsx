...
import { selectProjectFamilies } from '@/utils/canvasFormats'
...
export function ContentSyncModal({ open, onClose }: ContentSyncModalProps) {
  ...
  return <ModalShell open={open} onClose={onClose} title="Bring content from" maxWidth="max-w-md" bodyClassName="px-5 py-5" footerClassName="flex justify-end gap-2 border-t border-[rgba(255,255,255,0.06)] px-5 py-4" footer={<><button type="button" onClick={onClose} className="rounded-lg px-3 py-2 text-xs text-[#8f90a3] hover:bg-[rgba(255,255,255,0.06)] hover:text-white">Cancel</button><button type="button" onClick={handleConfirm} disabled={!plan || plan.updatedCount === 0} className="rounded-lg bg-[#7c6ef6] px-3 py-2 text-xs font-medium text-white hover:bg-[#6c5ed6] disabled:cursor-not-allowed disabled:opacity-50">Bring content</button></>}>
    {candidates.length === 1 && selectedFamily && <p className="text-sm leading-6 text-[#b7b7c5]">Bring content from <span className="font-medium text-[#f4f4f7]">{familyLabels[selectedFamily]}</span></p>}
    {candidates.length > 1 && <label className="block text-[10px] font-semibold uppercase tracking-[0.15em] text-[#6b6b7a]">Bring content from<select value={selectedFamily} onChange={(event) => setSourceFamily(event.target.value as typeof candidates[number])} className="mt-2 w-full rounded border border-[rgba(255,255,255,0.1)] bg-[#0f0f13] px-2 py-1.5 text-xs font-normal normal-case tracking-normal text-[#e8e8f0] focus:outline-none">{candidates.map((family) => <option key={family} value={family}>{familyLabels[family]}</option>)}</select></label>}
    {sourceGroup && <p className="mt-4 text-sm leading-6 text-[#b7b7c5]">Pull from <span className="font-medium text-[#f4f4f7]">{familyLabels[selectedFamily!]} → “{sourceGroup.name}”</span></p>}
